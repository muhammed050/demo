import { Info, KeyRound, Maximize2, Settings, PlugZap } from 'lucide-react';
    ...
    <header className="h-14 border-b border-zinc-100 flex items-center justify-between px-6 bg-white shrink-0 relative z-40">
      <div className="flex items-center min-w-0">
        ...
        <div className="flex items-center gap-3 self-start mt-3 ml-2 min-w-0">
          ...
          <div className="flex items-center gap-3 min-w-0"><a href="https://x.com/arturitu" target="_blank" rel="noopener" className="text-[10px] font-medium text-zinc-400 hover:text-darkDelegation transition-colors truncate">@arturitu</a><a href="https://github.com/arturitu/the-delegation" target="_blank" rel="noopener" className="text-zinc-300 hover:text-darkDelegation transition-colors shrink-0" title="View on GitHub"><svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82 1.04-.82 1.53-.82 2.2-.82 2.2 0 .44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"/></svg></a></div>
        </div>
      </div>
      ...
      {isInfoOpen && <InfoModal key="info-modal" onClose={() => setIsInfoOpen(false)}/>} {isBYOKOpen && <BYOKModal key="byok-modal" onClose={() => setBYOKOpen(false)}/>} 
    </header>
  );
};
