...
function makeAgent(index:number, role:string, description:string, color:string):AgentNode {
  return { id:`custom-${role.toLowerCase()}-${index}`, index, name:role, description, color, model:DEFAULT_MODELS.text, humanInTheLoop:['Developer','Security','DevOps','Orchestrator'].includes(role), position:{x:(index-1)*180,y:280} };
}
...
export const TeamBuilderPanel:React.FC<{onClose:()=>void}> = ({onClose}) => {
  ...
  const [selected,setSelected]=useState<string[]>(['Researcher','Developer','SEO','QA']);
  ...
  const create=()=>{
    const leadRole=selected.includes('Orchestrator')?'Orchestrator':'Orchestrator';
    const lead:AgentNode={id:`custom-orchestrator-${Date.now()}`,index:1,name:`${leadRole} / Lead`,description:'Coordinates the team, delegates tasks, reviews tool results and asks for approval when required.',color:'#2563EB',model:DEFAULT_MODELS.text,humanInTheLoop:true,position:{x:0,y:130},subagents:agents};
