...
export const EXTERNAL_TOOL_DEFINITIONS = [
  ...
  { name:'github_edit_file', group:'github', description:'Propose a file edit. This always creates a human approval request before changing GitHub.', properties:{ repository:{type:'string'}, path:{type:'string'}, content:{type:'string'}, message:{type:'string'}, branch:{type:'string'} }, required:['repository','path','content','message'] },
...
export async function executeExternalTool(name:string,args:any):Promise<boolean> {
  ...
  try {
    ...
    window.dispatchEvent(new CustomEvent('delegation-tool-result',{detail:{name,args,error:error.message}}));
    return false;
  }
}
